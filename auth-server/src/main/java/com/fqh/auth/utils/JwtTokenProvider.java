...
public class JwtTokenProvider {
    ...
    public Authentication getAuthentication(String token) {
        try {
            ...
            log.error("获取已认证信息失败,jwt已过期");
            throw new ServiceException("获取已认证信息失败,jwt已过期");
        }
        return null;
    }
     ...
     * @return {@link String}
     */
    public String automaticRenewal(String token) {
        try {
            Claims claims = Jwts.parser()
                    .setSigningKey(Base64.getEncoder().encode(secret.getBytes(StandardCharsets.UTF_8)))
                    .parseClaimsJws(token)
                    .getBody();
            Date date = new Date();
            if (claims != null && (date.getTime() - claims.getExpiration().getTime()) >= 36000) {
                return createToken(claims.get("username").toString());
            }
        } catch (Exception e) {
            log.warn("token自动续签失败,exception:", e);
            throw new ServiceException("token自动续签失败");
        }
        return "";
    }
